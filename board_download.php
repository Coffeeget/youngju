...
if($ie){
    $file_name = iconv('utf-8','euc-kr',$file_name); //파일명의 문자셋 변경
}

//다운로드 하려는 파일인 $file_path가 존재하는지 검사
if(file_exists($file_path)){
    $fp = fopen($file_path,"rb"); //파일을 읽고 쓸수 있는 모드로 열어~
...
//이함수는 false반환 실패한 경우 문자 수가, 송신 반환됨.(즉, 반환에 실패하지 않는 경우 파일 닫기 실행되도록~)
if(!fpassthru($fp))
    fclose();
?>